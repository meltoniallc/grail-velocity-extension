...
(function (global) {
  ...
  function create(adapter) {
    ...
    async function putScan(scan) {
      ...
      const keptSold = rows.filter((r) => r.keep && r.kind !== "active");
      const parked = rows.filter((r) => !r.keep || r.kind === "active");
      ...
      for (const c of keptSold) compsById.set(c.id, compactComp(c));
      ...
      const snap = {
        ...
        nCliff: verified.nCliff || parked.filter((d) => (d.flags || []).includes("beyond_cliff")).length,
        median: priced.median,
        p25: priced.p25,
        p75: priced.p75,
        fastCash: comps.length ? priced.fastCashPrice ?? null : existing?.fastCash ?? null,
        at: Date.now(),
        url: scan.url || null,
      };
      ...
      const scratchEntry = {
        ...
        listings: parked.map((d) => ({
          ...
          flags: d.flags,
          reason: d.reason,
          kind: d.kind || (d.keep ? "active" : "dropped"),
          keep: Boolean(d.keep),
        })),
      };
